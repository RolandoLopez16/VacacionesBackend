...
export function toUtc(value: LocalDate): Date {
  const [y, m, d] = value.split("-").map(Number);
  return new Date(Date.UTC(y!, m! - 1, d!));
}
export function fromUtc(value: Date): LocalDate {
...
export function daysBetween(start: LocalDate, end: LocalDate): number {
  return Math.round((toUtc(end).getTime() - toUtc(start).getTime()) / 86400000);
}
...
export function addYearsAnniversary(value: LocalDate, years: number): LocalDate {
  const [y, m, d] = value.split("-").map(Number);
