...
import {
  normalizePendingPeriodRows,
  type PendingPeriodRawRow,
} from "./pendingPeriodImport.js";
...
export class VacationService {
  ...
  async detail(
    ...
    const periodDtos: VacationPeriodDto[] = periods.map((raw) => {
      ...
      return {
        ...
        displayStatus,
        ...(p.closureType ? { closureType: p.closureType } : {}),
        ...(p.closureObservation
          ? { closureObservation: p.closureObservation }
          : {}),
        ...(p.closureAccountingDocument
          ? { closureAccountingDocument: p.closureAccountingDocument }
          : {}),
        ...(p.closureAmountCOP !== undefined
          ? { closureAmountCOP: p.closureAmountCOP }
          : {}),
      ...
      .map((employment) => {
        ...
        const periodItems = periods.map((period) => {
          ...
          return {
            ...
            pendingDays: state === "PENDING_LIQUIDATION"
        ...
        return {
          ...
          documentNumber: worker?.documentNumber ?? "—",
          endDate: employment.endDate,
          pendingPeriods: periodItems.filter(
            (period) => period.state === "PENDING_LIQUIDATION",
          ).length,
          enjoyedPeriods: periodItems.filter(
            (period) => period.state === "ENJOYED",
          ).length,
          liquidatedPeriods: periodItems.filter(
            (period) => period.state === "LIQUIDATED",
          ).length,
          closedPeriods: periodItems.filter(
  ...
  async closeRetiredEmploymentsWithAccounting(
    ...
    if (!input.accountingDocument.trim())
      throw failure("El documento contable es obligatorio");
    if (input.observation.trim().length < 3)
      throw failure("La observación debe tener al menos 3 caracteres");
    if (input.amountCOP !== undefined && input.amountCOP < 0)
      throw failure("El valor contable no puede ser negativo");
    const policy = await this.store.current(asOf);
    const employments = (await this.store.listEmployments()).filter(
      (employment) =>
        employment.status === "RETIRED" &&
    ...
    for (const employment of employments) {
      ...
      const changed = periods
        ...
        .map((period) => ({
          ...
          ...(input.amountCOP !== undefined
            ? { closureAmountCOP: input.amountCOP }
            : {}),
      ...
      const persistedPeriods = periods.map(
        (period) => changedById.get(period.id) ?? period,
      );
    ...
    const plans: VacationPeriodClosurePlan[] = periods
      .map((period) => {
        ...
        if (!employment || !worker) {
          ...
          decision = "PROTECTED";
          reason = "Período de 2015 o anterior conservado como pendiente histórico";
  ...
  private async buildPendingPeriodImportPlan(
    ...
    const now = new Date().toISOString();
    const observation = "Liquidación en sistema contable";
    ...
    for (const line of normalized.lines) {
      ...
      const exact = workerEmployments.filter(
        (employment) => employment.startDate === line.hireDate,
      );
      const compatible = workerEmployments.filter(
        (employment) =>
          employment.startDate <= line.hireDate &&
          (!employment.endDate || employment.endDate >= line.hireDate),
      );
      const employment =
        exact.length === 1
          ? exact[0]
      ...
      if (!employment) {
        errors.push({
          ...
          message: workerEmployments.length
            ? `No fue posible determinar un contrato único para ${line.employee}; valide Fecha Ing.`
      ...
      const causedOpen = generated
        .filter(
          (period) =>
            period.causedAt <= asOf && period.lifecycleStatus !== "CLOSED",
        )
      ...
      const keepIds = new Set(
        causedOpen.slice(0, line.pendingPeriods).map((period) => period.id),
      );
      for (const period of generated) {
        ...
        if (period.lifecycleStatus === "CLOSED") {
          ...
          decision = "PROTECTED";
          reason = "Período protegido por una carga anterior de pendientes";
          updated = protect(period);
        } else if (period.accrualStartDate <= "2015-12-31") {
          decision = "PROTECTED";
          reason = "Período de 2015 o anterior conservado como pendiente histórico";
          updated = protect(period);
          ...
          decision = "PROTECTED";
          reason = "Tiene una liquidación disfrutada o compensada activa; no se cierra automáticamente";
          updated = protect(period);
          ...
          decision = "CLOSE";
          reason = "Período causado no incluido dentro de los períodos pendientes reportados";
          updated = {
            ...period,
            lifecycleStatus: "CLOSED",
    ...
    return {
      sourceLines: normalized.lines.map(
  ...
  async previewPendingPeriodImport(
    ...
    const existing = await this.store.findVacationPendingPeriodImportByFileHash(
      fileHash,
    );
    if (existing?.status === "APPLIED")
    ...
    await this.store.saveVacationPendingPeriodImportBatch(batch);
    return { alreadyProcessed: false, batch, plans: batch.plans };
  }
  ...
  async applyPendingPeriodImport(
    ...
    const batch = await this.store.findVacationPendingPeriodImportBatch(batchId);
    if (!batch) throw failure("No existe la vista previa de períodos pendientes", 404);
    if (
    ...
    if (
      built.errors.length ||
      built.plans.some((plan) => plan.decision === "REVIEW")
    )
      throw failure(
        "Los datos cambiaron desde la vista previa; genera una nueva vista previa",
        409,
      );
    const currentPeriods = await this.store.findByEmploymentIds(
      [...new Set(batch.plans.map((plan) => plan.employmentId))],
    );
    ...
    const appliedBatch: VacationPendingPeriodImportBatch = {
      ...
      closedPeriods: built.plans.filter((plan) => plan.decision === "CLOSE").length,
    ...
    const planByPeriodId = new Map(
      built.plans.map((plan) => [plan.periodId, plan]),
    );
    const appliedPeriods = built.periods.map((period) => {
      const plan = planByPeriodId.get(period.id);
      return plan?.decision === "CLOSE"
        ? { ...period, closedAt: appliedAt, closedBy: actor }
        : period;
    });
    const audits = [
      ...built.plans
        .filter((plan) => plan.decision === "CLOSE" || plan.decision === "PROTECTED" || plan.decision === "KEEP")
        .map((plan) => ({
          id: crypto.randomUUID(),
          actorId: actor,
          action:
            plan.decision === "CLOSE"
              ? "VACATION_PERIOD_CLOSED_BY_PENDING_IMPORT"
        ...
        })),
      {
        ...
        metadata: {
          ...
          closedPeriods: appliedBatch.closedPeriods,
    ...
    await this.store.applyVacationPendingPeriodImport(
      appliedBatch,
      appliedPeriods,
      audits,
    );
    ...
    for (const plan of plans) {
      for (const period of plan.periods) {
        if (
          ...
          period.accrualStartDate > "2015-12-31" &&
          period.lifecycleStatus !== "CLOSED" &&
          !period.pendingImportProtected
        )
